...
if [ "$USE_Paper" = "yes" ] ; then
  ...
  # rm $installer_jar
  echo "cd ServerMC && java -Xmx3G -DPaper.IgnoreJavaVersion=true -jar paper-1.20.4-405.jar nogui" > ../m.sh
else
  wget -O $EXEC_SERVER_NAME $VANILLA_SERVER
  echo "cd ServerMC && java -Xmx1G -jar ${EXEC_SERVER_NAME} nogui" > ../m.sh
fi
chmod +x ../m.sh
